...
/** Description @Author: cjl @Date: 2019/1/22 10:29 */
@RestController
...
public class SerialNumberController {
  ...
  /**
   * create by: cjl description: 批量添加序列号 create time: 2019/1/29 15:11 @Param: materialName @Param:
   * serialNumberPrefix @Param: batAddTotal @Param: remark
   *
  ...
  public String batAddSerialNumber(@RequestBody JSONObject jsonObject, HttpServletRequest request)
    ...
    int insert =
        serialNumberService.batAddSerialNumber(
            materialCode, serialNumberPrefix, batAddTotal, remark);
    if (insert > 0) {
  ...
  public BaseResponseInfo getEnableSerialNumberList(
    ...
    try {
      ...
      List<SerialNumberEx> list =
          serialNumberService.getEnableSerialNumberList(
              number, name, depotId, barCode, (currentPage - 1) * pageSize, pageSize);
