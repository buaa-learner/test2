...
public class MaterialController {
  ...
  public String checkIsExist(
    ...
    int exist =
        materialService.checkIsExist(
            id,
            name,
            StringUtil.toNull(model),
            StringUtil.toNull(color),
            StringUtil.toNull(standard),
            StringUtil.toNull(mfrs),
            StringUtil.toNull(otherField1),
            StringUtil.toNull(otherField2),
            StringUtil.toNull(otherField3),
            StringUtil.toNull(unit),
            unitId);
    if (exist > 0) {
  ...
  public String batchSetStatus(@RequestBody JSONObject jsonObject, HttpServletRequest request)
      throws Exception {
  ...
  public BaseResponseInfo findByIdWithBarCode(
  ...
  public BaseResponseInfo getMaterialByParam(
      @RequestParam("q") String q, HttpServletRequest request) throws Exception {
    BaseResponseInfo res = new BaseResponseInfo();
    try {
  ...
  public JSONObject findBySelect(
    ...
    try {
      ...
      List<MaterialVo4Unit> dataList =
          materialService.findBySelectWithBarCode(
              categoryId,
              q,
              enableSerialNumber,
              enableBatchNumber,
              ...
              pageSize);
      int total =
          materialService.findBySelectWithBarCodeCount(
              categoryId, q, enableSerialNumber, enableBatchNumber);
  ...
  public JSONObject getMaterialByMeId(
      @RequestParam(value = "meId", required = false) Long meId,
      @RequestParam("mpList") String mpList,
      HttpServletRequest request)
      throws Exception {
    JSONObject item = new JSONObject();
    try {
      ...
      if (materialList != null && materialList.size() != 1) {
        ...
        MaterialName =
            MaterialName
                + material.getmBarCode()
                + "_"
                + material.getName()
                + ((material.getStandard() == null || material.getStandard().equals(""))
                    ? ""
        ...
        MaterialName =
            MaterialName
                + expand
                + ((material.getUnit() == null || material.getUnit().equals(""))
                    ? ""
                    : "(" + material.getUnit() + ")")
                + ratio;
  ...
  public void exportExcel(
    ...
    try {
      ...
      List<MaterialVo4Unit> dataList =
          materialService.exportExcel(
              StringUtil.toNull(materialParam),
              StringUtil.toNull(color),
              StringUtil.toNull(weight),
              StringUtil.toNull(expiryNum),
              StringUtil.toNull(enabled),
              StringUtil.toNull(enableSerialNumber),
              StringUtil.toNull(enableBatchNumber),
              StringUtil.toNull(remark),
              StringUtil.toNull(categoryId));
      String[] names = {
        "条码", "名称", "规格", "型号", "颜色", "类别", "扩展信息", "单位", "基础重量", "保质期", "采购价", "零售价", "销售价",
        "最低售价", "备注", "状态", "序列号", "批号"
      };
      String title = "商品信息";
      ...
      if (null != dataList) {
        for (MaterialVo4Unit m : dataList) {
          ...
          objs[10] =
              m.getPurchaseDecimal() == null
                  ? ""
                  : m.getPurchaseDecimal().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[11] =
              m.getCommodityDecimal() == null
                  ? ""
                  : m.getCommodityDecimal().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[12] =
              m.getWholesaleDecimal() == null
                  ? ""
                  : m.getWholesaleDecimal().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[13] =
              m.getLowDecimal() == null
                  ? ""
                  : m.getLowDecimal().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[14] = m.getRemark();
  ...
  public BaseResponseInfo importExcel(
  ...
  public JSONObject getMaterialEnableSerialNumberList(
    ...
    JSONObject object = new JSONObject();
    try {
      List<MaterialVo4Unit> list =
          materialService.getMaterialEnableSerialNumberList(
              q, (currentPage - 1) * pageSize, pageSize);
  ...
  public BaseResponseInfo getMaterialByBarCode(
    ...
    try {
      ...
      if (list != null && list.size() > 0) {
        for (MaterialVo4Unit mvo : list) {
          ...
          } else if ("CGDD".equals(prefixNo)
              || "CGRK".equals(prefixNo)
              || "CGTH".equals(prefixNo)
              || "QTRK".equals(prefixNo)
              || "DBCK".equals(prefixNo)
              || "ZZD".equals(prefixNo)
              || "CXD".equals(prefixNo)
              || "PDLR".equals(prefixNo)
              || "PDFP".equals(prefixNo)) {
            // 采购价
            mvo.setBillPrice(mvo.getPurchaseDecimal());
          } else if ("XSDD".equals(prefixNo)
              || "XSCK".equals(prefixNo)
              || "XSTH".equals(prefixNo)
  ...
  public BaseResponseInfo getListWithStock(
    ...
    try {
      ...
      List<MaterialVo4Unit> dataList =
          materialService.getListWithStock(
              depotList,
              idList,
              StringUtil.toNull(materialParam),
              zeroStock,
              StringUtil.safeSqlParse(column),
              StringUtil.safeSqlParse(order),
              (currentPage - 1) * pageSize,
              pageSize);
      int total =
          materialService.getListWithStockCount(
              depotList, idList, StringUtil.toNull(materialParam), zeroStock);
      MaterialVo4Unit materialVo4Unit =
          materialService.getTotalStockAndPrice(
              depotList, idList, StringUtil.toNull(materialParam));
      map.put("total", total);
  ...
  public String batchUpdate(@RequestBody JSONObject jsonObject, HttpServletRequest request)
      throws Exception {
    Map<String, Object> objectMap = new HashMap<>();
