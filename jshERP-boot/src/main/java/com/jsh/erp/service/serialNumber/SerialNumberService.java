...
/** Description @Author: cjl @Date: 2019/1/21 16:33 */
@Service
public class SerialNumberService {
  private Logger logger = LoggerFactory.getLogger(SerialNumberService.class);

  @Resource private SerialNumberMapper serialNumberMapper;
  ...
  public List<SerialNumberEx> select(
      String serialNumber, String materialName, Integer offset, Integer rows) throws Exception {
    return null;
  }
  ...
  public int insertSerialNumber(JSONObject obj, HttpServletRequest request) throws Exception {
    ...
    try {
      SerialNumberEx serialNumberEx =
          JSONObject.parseObject(obj.toJSONString(), SerialNumberEx.class);
      /** 处理商品id */
      serialNumberEx.setMaterialId(
          getSerialNumberMaterialIdByBarCode(serialNumberEx.getMaterialCode()));
      // 删除标记,默认未删除
  ...
  /**
   * create by: qiankunpingtai 逻辑删除序列号信息 create time: 2019/3/27 17:43 @Param: ids
   *
   * @return
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  public int batchDeleteSerialNumberByIds(String ids) throws Exception {
    ...
    try {
      result =
          serialNumberMapperEx.batchDeleteSerialNumberByIds(
  ...
  public int checkIsNameExist(Long id, String serialNumber) throws Exception {
    ...
    example
        .createCriteria()
        .andIdNotEqualTo(id)
        .andSerialNumberEqualTo(serialNumber)
  ...
  /**
   * create by: cjl description: 根据商品名称判断商品名称是否有效 create time: 2019/1/23 17:04 @Param: materialName
   *
    ...
    if (StringUtil.isNotEmpty(materialName)) {
      ...
      if (mlist == null || mlist.size() < 1) {
        ...
        throw new BusinessRunTimeException(
            ExceptionConstants.MATERIAL_NOT_EXISTS_CODE,
            ExceptionConstants.MATERIAL_NOT_EXISTS_MSG);
      }
      if (mlist.size() > 1) {
        // 商品信息不唯一
        throw new BusinessRunTimeException(
            ExceptionConstants.MATERIAL_NOT_ONLY_CODE, ExceptionConstants.MATERIAL_NOT_ONLY_MSG);
      }
      // 获得唯一商品
      if (BusinessConstants.ENABLE_SERIAL_NUMBER_NOT_ENABLED.equals(
          mlist.get(0).getEnableSerialNumber())) {
        // 商品未开启序列号
        throw new BusinessRunTimeException(
            ExceptionConstants.MATERIAL_NOT_ENABLE_SERIAL_NUMBER_CODE,
  ...
  /**
   * create by: cjl description: 根据商品名称判断给商品添加序列号是否可行 1、根据商品名称必须查询到唯一的商品 2、该商品必须已经启用序列号
   * 3、该商品已绑定序列号数量小于商品现有库存 2019-02-01 用商品的库存去限制序列号的添加有点不合乎道理，去掉此限制 create time: 2019/1/23
   * 17:04 @Param: materialName
   *
   * @return Long 满足使用条件的商品的id
   */
  ...
  /**
   * create by: cjl description: 出库时判断序列号库存是否足够， 同时将对应的序列号绑定单据 create time: 2019/1/24 16:24 @Param:
   * List<DepotItem>
   *
   * @return void
   */
  public void checkAndUpdateSerialNumber(
      DepotItem depotItem, String outBillNo, User userInfo, String snList) throws Exception {
    if (depotItem != null) {
  ...
  /** */
  /**
   * create by: cjl description: 卖出序列号 create time: 2019/1/25 9:17 @Param: materialId @Param:
   * depotheadId @Param: isSell 卖出'1' @Param: Count 卖出或者赎回的数量
   *
  ...
  public int sellSerialNumber(Long materialId, String outBillNo, String snList, User user)
    ...
    try {
      ...
      result =
          serialNumberMapperEx.sellSerialNumber(
              materialId, outBillNo, snArray, new Date(), user == null ? null : user.getId());
    } catch (Exception e) {
      JshException.writeFail(logger, e);
    }
    return result;
  ...
  /**
   * create by: cjl description: 赎回序列号 create time: 2019/1/25 9:17 @Param: materialId  * @Param:
   * depotheadId  * @Param: isSell 赎回'0'  * @Param: Count 卖出或者赎回的数量
   *
   * @return com.jsh.erp.datasource.entities.SerialNumberEx
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  public int cancelSerialNumber(Long materialId, String outBillNo, int count, User user)
    ...
    try {
      result =
          serialNumberMapperEx.cancelSerialNumber(
              materialId, outBillNo, count, new Date(), user == null ? null : user.getId());
    } catch (Exception e) {
      JshException.writeFail(logger, e);
    }
    return result;
  ...
  /**
   * create by: cjl description: 批量添加序列号，最多500个 create time: 2019/1/29 15:11 @Param:
   * materialName @Param: serialNumberPrefix @Param: batAddTotal @Param: remark
   *
   * @return java.lang.Object
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  public int batAddSerialNumber(
      String materialCode, String serialNumberPrefix, Integer batAddTotal, String remark)
      throws Exception {
    int result = 0;
    try {
      if (StringUtil.isNotEmpty(materialCode)) {
        ...
        int forNum =
            BusinessConstants.BATCH_INSERT_MAX_NUMBER >= batAddTotal
                ? batAddTotal
                : BusinessConstants.BATCH_INSERT_MAX_NUMBER;
        ...
        logService.insertLog(
            "序列号",
            new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_BATCH_ADD)
                .append(batAddTotal)
  ...
  public List<SerialNumberEx> getEnableSerialNumberList(
      String number, String name, Long depotId, String barCode, Integer offset, Integer rows)
      throws Exception {
    List<SerialNumberEx> list = null;
    try {
      list =
          serialNumberMapperEx.getEnableSerialNumberList(
              StringUtil.toNull(number), StringUtil.toNull(name), depotId, barCode, offset, rows);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return list;
  ...
  public Long getEnableSerialNumberCount(String number, String name, Long depotId, String barCode)
    ...
    try {
      count =
          serialNumberMapperEx.getEnableSerialNumberCount(
              StringUtil.toNull(number), StringUtil.toNull(name), depotId, barCode);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return count;
  ...
  public void addSerialNumberByBill(
      String type, String subType, String inBillNo, Long materialId, Long depotId, String snList)
      throws Exception {
    // 录入序列号的时候不能重复
    if ((BusinessConstants.SUB_TYPE_PURCHASE.equals(subType)
      ...
      for (String sn : snArr) {
        ...
        example
            ...
            .andSerialNumberEqualTo(sn.trim())
            .andIsSellEqualTo("0")
            .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
        list = serialNumberMapper.selectByExample(example);
        // 判断如果不存在重复序列号就新增
        if (list == null || list.size() == 0) {
          ...
          if (!inBillNo.equals(list.get(0).getInBillNo())) {
            throw new BusinessRunTimeException(
                ExceptionConstants.SERIAL_NUMBERE_ALREADY_EXISTS_CODE,
