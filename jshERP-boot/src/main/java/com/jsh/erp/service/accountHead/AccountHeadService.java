...
public class AccountHeadService {
  ...
  public List<AccountHeadVo4ListEx> select(
      String type,
      String roleType,
      String billNo,
      String beginTime,
    ...
    try {
      ...
      List<AccountHeadVo4ListEx> list =
          accountHeadMapperEx.selectByConditionAccountHead(
              type,
              creatorArray,
              billNo,
              beginTime,
  ...
  public Long countAccountHead(
    ...
    try {
      ...
      endTime = Tools.parseDayToTime(endTime, BusinessConstants.DAY_LAST_TIME);
      result =
          accountHeadMapperEx.countsByAccountHead(
              type,
              creatorArray,
              billNo,
              beginTime,
  ...
  public int batchDeleteAccountHeadByIds(String ids) throws Exception {
      ...
      if ("收预付款".equals(accountHead.getType())) {
        if (accountHead.getOrganId() != null) {
          ...
          supplierService.updateAdvanceIn(
              accountHead.getOrganId(), BigDecimal.ZERO.subtract(accountHead.getTotalPrice()));
        }
      }
    }
    User userInfo = userService.getCurrentUser();
    ...
    // 删除主表
    accountItemMapperEx.batchDeleteAccountItemByHeadIds(
        new Date(), userInfo == null ? null : userInfo.getId(), idArray);
    // 删除子表
    accountHeadMapperEx.batchDeleteAccountHeadByIds(
        new Date(), userInfo == null ? null : userInfo.getId(), idArray);
    logService.insertLog(
        "财务",
        sb.toString(),
  ...
  public void addAccountHeadAndDetail(String beanJson, String rows, HttpServletRequest request)
    ...
    if (checkIsBillNoExist(0L, accountHead.getBillNo()) > 0) {
      throw new BusinessRunTimeException(
          ExceptionConstants.ACCOUNT_HEAD_BILL_NO_EXIST_CODE,
    ...
    dhExample
        .createCriteria()
        .andBillNoEqualTo(accountHead.getBillNo())
        .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
  ...
  public void updateAccountHeadAndDetail(String beanJson, String rows, HttpServletRequest request)
    ...
    if ("收预付款".equals(accountHead.getType())) {
      supplierService.updateAdvanceIn(
          accountHead.getOrganId(), accountHead.getTotalPrice().subtract(preTotalPrice));
    }
    logService.insertLog(
        "财务单据",
        new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_EDIT)
