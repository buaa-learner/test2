...
public class LogService {
  ...
  @Resource private LogMapperEx logMapperEx;
  ...
  public List<LogVo4List> select(
    ...
    try {
      ...
      list =
          logMapperEx.selectByConditionLog(
              operation, userInfo, clientIp, status, beginTime, endTime, content, offset, rows);
      if (null != list) {
  ...
  public Long countLog(
    ...
    try {
      ...
      endTime = Tools.parseDayToTime(endTime, BusinessConstants.DAY_LAST_TIME);
      result =
          logMapperEx.countsByLog(
              operation, userInfo, clientIp, status, beginTime, endTime, content);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return result;
  ...
  public void insertLogWithUserId(
      Long userId, Long tenantId, String moduleName, String content, HttpServletRequest request)
      throws Exception {
    try {
      if (userId != null) {
        Log log = new Log();
