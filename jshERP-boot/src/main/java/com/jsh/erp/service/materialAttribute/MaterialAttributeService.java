...
public class MaterialAttributeService {
  ...
  @Resource private MaterialAttributeMapper materialAttributeMapper;

  @Resource private MaterialAttributeMapperEx materialAttributeMapperEx;
  ...
  public List<MaterialAttribute> select(String attributeName, int offset, int rows)
    ...
    try {
      list =
          materialAttributeMapperEx.selectByConditionMaterialAttribute(attributeName, offset, rows);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return list;
  ...
  public int updateMaterialAttribute(JSONObject obj, HttpServletRequest request) throws Exception {
    MaterialAttribute materialAttribute =
        JSONObject.parseObject(obj.toJSONString(), MaterialAttribute.class);
    try {
      ...
      logService.insertLog(
          ...
          new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_EDIT)
              .append(materialAttribute.getAttributeName())
              .toString(),
          request);
      return 1;
    } catch (Exception e) {
