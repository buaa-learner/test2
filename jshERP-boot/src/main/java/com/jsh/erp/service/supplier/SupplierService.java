...
public class SupplierService {
  ...
  @Resource private SupplierMapperEx supplierMapperEx;
  @Resource private LogService logService;
  @Resource private UserService userService;
  @Resource private AccountHeadMapperEx accountHeadMapperEx;
  @Resource private DepotHeadMapperEx depotHeadMapperEx;
  @Resource private AccountItemMapperEx accountItemMapperEx;
  ...
  public List<Supplier> select(
      String supplier, String type, String phonenum, String telephone, int offset, int rows)
      throws Exception {
    List<Supplier> resList = new ArrayList<Supplier>();
    try {
      List<Supplier> list =
          supplierMapperEx.selectByConditionSupplier(
              supplier, type, phonenum, telephone, offset, rows);
      for (Supplier s : list) {
        ...
        sum =
            sum.add(depotHeadService.findTotalPay(supplierId, endTime, supType))
  ...
  public Long countSupplier(String supplier, String type, String phonenum, String telephone)
      throws Exception {
    Long result = null;
    try {
  ...
  public int insertSupplier(JSONObject obj, HttpServletRequest request) throws Exception {
    ...
    try {
      ...
      if ("客户".equals(supplier.getType())) {
        ...
        Supplier sInfo =
            supplierMapperEx.getSupplierByNameAndType(supplier.getSupplier(), supplier.getType());
        String ubKey = "[" + sInfo.getId() + "]";
        List<UserBusiness> ubList =
            userBusinessService.getBasicData(userId.toString(), "UserCustomer");
  ...
  public int batchDeleteSupplierByIds(String ids) throws Exception {
    ...
    try {
      result =
          supplierMapperEx.batchDeleteSupplierByIds(
  ...
  public void importVendor(MultipartFile file, HttpServletRequest request) throws Exception {
    ...
    // '名称', '联系人', '手机号码', '联系电话', '电子邮箱', '传真', '期初应付', '纳税人识别号', '税率(%)', '开户行', '账号', '地址',
    // '备注', '排序', '状态'
  ...
  public void importCustomer(MultipartFile file, HttpServletRequest request) throws Exception {
    ...
    Workbook workbook = Workbook.getWorkbook(file.getInputStream());
    Sheet src = workbook.getSheet(0);
    // '名称', '联系人', '手机号码', '联系电话', '电子邮箱', '传真', '期初应收', '纳税人识别号', '税率(%)', '开户行', '账号', '地址',
    // '备注', '排序', '状态'
    List<Supplier> sList = new ArrayList<>();
    for (int i = 2; i < src.getRows(); i++) {
      String supplierName = ExcelUtils.getContent(src, i, 0);
  ...
  public BaseResponseInfo importExcel(List<Supplier> mList, String type) throws Exception {
    ...
    try {
      for (Supplier s : mList) {
        ...
        example
            .createCriteria()
            .andSupplierEqualTo(s.getSupplier())
            .andTypeEqualTo(type)
            .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
      ...
      if (null != dataList) {
        for (Supplier s : dataList) {
          ...
          objs[5] =
              s.getAdvanceIn() == null
                  ? ""
                  : s.getAdvanceIn().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
  ...
  private File exportExcelVendorOrCustomer(List<Supplier> dataList, String type) throws Exception {
    ...
    if (null != dataList) {
      for (Supplier s : dataList) {
        ...
        if (("客户").equals(s.getType())) {
          objs[6] =
              s.getBeginNeedGet() == null
                  ? ""
                  : s.getBeginNeedGet().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[7] =
              s.getAllNeedGet() == null
                  ? ""
                  : s.getAllNeedGet().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        } else if (("供应商").equals(s.getType())) {
          objs[6] =
              s.getBeginNeedPay() == null
                  ? ""
                  : s.getBeginNeedPay().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
          objs[7] =
              s.getAllNeedPay() == null
                  ? ""
                  : s.getAllNeedPay().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        }
        objs[8] = s.getTaxNum();
        objs[9] =
            s.getTaxRate() == null
                ? ""
                : s.getTaxRate().setScale(2, BigDecimal.ROUND_HALF_UP).toString();
