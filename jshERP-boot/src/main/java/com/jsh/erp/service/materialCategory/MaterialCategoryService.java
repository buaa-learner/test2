...
public class MaterialCategoryService {
  ...
  @Resource private MaterialCategoryMapper materialCategoryMapper;
  ...
  public List<MaterialCategory> select(String name, Integer parentId, int offset, int rows)
    ...
    try {
      list =
          materialCategoryMapperEx.selectByConditionMaterialCategory(name, parentId, offset, rows);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return list;
  ...
  public int insertMaterialCategory(JSONObject obj, HttpServletRequest request) throws Exception {
    MaterialCategory materialCategory =
        JSONObject.parseObject(obj.toJSONString(), MaterialCategory.class);
    ...
    try {
      ...
      logService.insertLog(
          ...
          new StringBuffer(BusinessConstants.LOG_OPERATION_TYPE_ADD)
              .append(materialCategory.getName())
              .toString(),
          request);
    } catch (Exception e) {
  ...
  public int batchDeleteMaterialCategoryByIds(String ids) throws Exception {
    ...
    List<MaterialCategory> mcList =
        materialCategoryMapperEx.getMaterialCategoryListByCategoryIds(idArray);
    if (mcList != null && mcList.size() > 0) {
      logger.error(
          "异常码[{}],异常提示[{}]",
          ExceptionConstants.MATERIAL_CATEGORY_CHILD_NOT_SUPPORT_DELETE_CODE,
      ...
      result =
          materialCategoryMapperEx.batchDeleteMaterialCategoryByIds(updateDate, updater, strArray);
    }
    return result;
  }
  ...
  /**
   * create by: cjl description: 获取商品类别树数据 create time: 2019/2/19 14:30 @Param:
   *
  ...
  /**
   * create by: cjl description: 新增商品类别信息 create time: 2019/2/19 16:30 @Param: mc
   *
   * @return void
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  ...
  public void checkMaterialCategorySerialNo(MaterialCategory mc) throws Exception {
    ...
    if (mList.size() > 1) {
      // 查询到的数据条数大于1，编号已存在
      throw new BusinessRunTimeException(
          ExceptionConstants.MATERIAL_CATEGORY_SERIAL_ALREADY_EXISTS_CODE,
