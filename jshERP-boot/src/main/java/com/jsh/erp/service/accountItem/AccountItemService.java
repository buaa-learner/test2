...
public class AccountItemService {
  ...
  public void saveDetials(String rows, Long headerId, String type, HttpServletRequest request)
    ...
    if (null != rowArr && rowArr.size() > 0) {
      for (int i = 0; i < rowArr.size(); i++) {
        ...
        if (tempInsertedJson.get("accountId") != null
            && !tempInsertedJson.get("accountId").equals("")) {
          accountItem.setAccountId(tempInsertedJson.getLong("accountId"));
        }
        if (tempInsertedJson.get("inOutItemId") != null
            && !tempInsertedJson.get("inOutItemId").equals("")) {
          accountItem.setInOutItemId(tempInsertedJson.getLong("inOutItemId"));
        }
        if (tempInsertedJson.get("billNumber") != null
            && !tempInsertedJson.get("billNumber").equals("")) {
          String billNo = tempInsertedJson.getString("billNumber");
          accountItem.setBillId(depotHeadService.getDepotHead(billNo).getId());
        }
        if (tempInsertedJson.get("needDebt") != null
            && !tempInsertedJson.get("needDebt").equals("")) {
          accountItem.setNeedDebt(tempInsertedJson.getBigDecimal("needDebt"));
        }
        if (tempInsertedJson.get("finishDebt") != null
            && !tempInsertedJson.get("finishDebt").equals("")) {
          accountItem.setFinishDebt(tempInsertedJson.getBigDecimal("finishDebt"));
        }
        if (tempInsertedJson.get("eachAmount") != null
            && !tempInsertedJson.get("eachAmount").equals("")) {
      ...
      throw new BusinessRunTimeException(
          ExceptionConstants.ACCOUNT_HEAD_ROW_FAILED_CODE,
  ...
  public int batchDeleteAccountItemByIds(String ids) throws Exception {
    ...
    try {
      result =
          accountItemMapperEx.batchDeleteAccountItemByIds(
