...
public class UserService {
  ...
  @Resource private UserMapperEx userMapperEx;
  ...
  /**
   * create by: cjl description: 添加事务控制 create time: 2019/1/11 14:30 @Param: beanJson  * @Param:
   * request
   *
   * @return int
   */
  ...
  /**
   * create by: cjl description: 添加事务控制 create time: 2019/1/11 14:31 @Param: beanJson  * @Param: id
   *
   * @return int
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  ...
  /**
   * create by: cjl description: 添加事务控制 create time: 2019/1/11 14:32 @Param: user
   *
   * @return int
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  ...
  /**
   * create by: cjl description: 添加事务控制 create time: 2019/1/11 14:33 @Param: md5Pwd  * @Param: id
   *
   * @return int
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  ...
  public int batDeleteUser(String ids) throws Exception {
    ...
    if (result < 1) {
      logger.error(
          "异常码[{}],异常提示[{}],参数,ids:[{}]",
          ExceptionConstants.USER_DELETE_FAILED_CODE,
          ExceptionConstants.USER_DELETE_FAILED_MSG,
          ids);
      throw new BusinessRunTimeException(
          ExceptionConstants.USER_DELETE_FAILED_CODE, ExceptionConstants.USER_DELETE_FAILED_MSG);
    }
    return result;
  }
      ...
      if (null != list && list.size() == 0) {
        ...
        if (tenant != null) {
          ...
          if (tenant.getExpireTime() != null
              && tenant.getExpireTime().getTime() < System.currentTimeMillis()) {
  ...
  /**
   * create by: cjl description: 获取当前用户信息 create time: 2019/1/24 10:01 @Param:
   *
  ...
  public User getCurrentUser() throws Exception {
    HttpServletRequest request =
        ((ServletRequestAttributes)
            ...
            .getRequest();
    Long userId =
        Long.parseLong(redisService.getObjectFromSessionByKey(request, "userId").toString());
  ...
  public void addUserAndOrgUserRel(UserEx ue, HttpServletRequest request) throws Exception {
    if (BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
      throw new BusinessRunTimeException(
          ExceptionConstants.USER_NAME_LIMIT_USE_CODE, ExceptionConstants.USER_NAME_LIMIT_USE_MSG);
    } else {
      logService.insertLog(
          "用户",
      ...
      if (ue == null) {
        logger.error(
            "异常码[{}],异常提示[{}],参数,[{}]",
            ExceptionConstants.USER_ADD_FAILED_CODE,
      ...
      if (ue.getOrgaId() != null && "1".equals(ue.getLeaderFlag())) {
        ...
        if (checkList.size() > 0) {
          throw new BusinessRunTimeException(
              ExceptionConstants.USER_LEADER_IS_EXIST_CODE,
      ...
      if (oul == null) {
        logger.error(
            "异常码[{}],异常提示[{}],参数,[{}]",
            ExceptionConstants.ORGA_USER_REL_ADD_FAILED_CODE,
  ...
  public UserEx addUser(UserEx ue) throws Exception {
    /** 新增用户默认设置 1、密码默认123456 2是否系统自带默认为非系统自带 3是否管理者默认为员工 4默认用户状态为正常 */
  ...
  public UserEx registerUser(UserEx ue, Integer manageRoleId, HttpServletRequest request)
      throws Exception {
    /** create by: qiankunpingtai create time: 2019/4/9 18:00 多次创建事务，事物之间无法协同，应该在入口处创建一个事务以做协调 */
    if (BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
      throw new BusinessRunTimeException(
          ExceptionConstants.USER_NAME_LIMIT_USE_CODE, ExceptionConstants.USER_NAME_LIMIT_USE_MSG);
    } else {
  ...
  public void updateUserAndOrgUserRel(UserEx ue, HttpServletRequest request) throws Exception {
    if (BusinessConstants.DEFAULT_MANAGER.equals(ue.getLoginName())) {
      ...
      if (ue == null) {
        logger.error(
            "异常码[{}],异常提示[{}],参数,[{}]",
            ExceptionConstants.USER_EDIT_FAILED_CODE,
  ...
  /**
   * 检查登录名不能重复 create time: 2019/3/12 11:36 @Param: userEx
   *
   * @return void
   */
  ...
  public JSONArray getBtnStrArrById(Long userId) throws Exception {
    ...
    List<UserBusiness> userRoleList =
        userBusinessService.getBasicData(userId.toString(), "UserRole");
    if (userRoleList != null && userRoleList.size() > 0) {
      ...
      if (StringUtil.isNotEmpty(roleValue)
        ...
        List<UserBusiness> roleFunctionsList =
            userBusinessService.getBasicData(roleValue, "RoleFunctions");
  ...
  public int batchSetStatus(Byte status, String ids, HttpServletRequest request) throws Exception {
    ...
    long userNumLimit =
        Long.parseLong(redisService.getObjectFromSessionByKey(request, "userNumLimit").toString());
    if (selectUserSize + enableUserSize > userNumLimit && status == 0) {
      throw new BusinessParamCheckingException(
          ExceptionConstants.USER_ENABLE_OVER_LIMIT_FAILED_CODE,
