...
public class DepotItemService {
  ...
  @Resource private DepotItemMapper depotItemMapper;
  @Resource private DepotItemMapperEx depotItemMapperEx;
  @Resource private MaterialService materialService;
  @Resource private MaterialExtendService materialExtendService;
  @Resource private SerialNumberMapperEx serialNumberMapperEx;
  ...
  @Resource private MaterialCurrentStockMapper materialCurrentStockMapper;
  ...
  public List<DepotItemVo4DetailByTypeAndMId> findDetailByDepotIdsAndMaterialIdList(
      String depotIds,
      Boolean forceFlag,
      String sku,
      String batchNumber,
    ...
    try {
      list =
          depotItemMapperEx.findDetailByDepotIdsAndMaterialIdList(
              depotIdArray,
              forceFlag,
              sku,
              batchNumber,
  ...
  public Long findDetailByDepotIdsAndMaterialIdCount(
      String depotIds,
      Boolean forceFlag,
      String sku,
      String batchNumber,
    ...
    try {
      result =
          depotItemMapperEx.findDetailByDepotIdsAndMaterialIdCount(
              depotIdArray, forceFlag, sku, batchNumber, number, beginTime, endTime, mId);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return result;
  ...
  public DepotItem getItemByHeaderIdAndMaterial(Long headerId, Long meId) throws Exception {
    ...
    try {
      ...
      example
          .createCriteria()
          .andHeaderIdEqualTo(headerId)
          .andMaterialExtendIdEqualTo(meId)
          .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
  ...
  public DepotItem getPreItemByHeaderIdAndMaterial(String linkNumber, Long meId, Long linkId)
    ...
    try {
      ...
      example
          .createCriteria()
          .andHeaderIdEqualTo(depotHead.getId())
          .andMaterialExtendIdEqualTo(meId)
          .andIdEqualTo(linkId)
          .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
      List<DepotItem> list = depotItemMapper.selectByExample(example);
  ...
  public List<DepotItemVo4WithInfoEx> findByAll(
      String materialParam, String endTime, Integer offset, Integer rows) throws Exception {
    List<DepotItemVo4WithInfoEx> list = null;
    try {
  ...
  public List<DepotItemVo4WithInfoEx> getListWithBugOrSale(
    ...
    try {
      list =
          depotItemMapperEx.getListWithBugOrSale(
              materialParam,
              billType,
              beginTime,
              endTime,
  ...
  public int getListWithBugOrSaleCount(
      String materialParam,
      String billType,
      String beginTime,
      String endTime,
    ...
    try {
      result =
          depotItemMapperEx.getListWithBugOrSaleCount(
              materialParam,
              billType,
              beginTime,
              endTime,
  ...
  public BigDecimal buyOrSale(
    ...
    try {
      if (SUM_TYPE.equals(sumType)) {
        result =
            depotItemMapperEx.buyOrSaleNumber(
                type,
                subType,
                MId,
                beginTime,
        ...
        result =
            depotItemMapperEx.buyOrSalePrice(
                type,
                subType,
                MId,
                beginTime,
  ...
  public BigDecimal inOrOutPrice(String type, String subType, String month, String roleType)
    ...
    try {
      ...
      result =
          depotItemMapperEx.inOrOutPrice(
              type, subType, beginTime, endTime, creatorArray, forceFlag);
    } catch (Exception e) {
      JshException.readFail(logger, e);
    }
    return result;
  ...
  public BigDecimal inOrOutRetailPrice(String type, String subType, String month, String roleType)
    ...
    try {
      ...
      String endTime = Tools.lastDayOfMonth(month) + BusinessConstants.DAY_LAST_TIME;
      result =
          depotItemMapperEx.inOrOutRetailPrice(
              type, subType, beginTime, endTime, creatorArray, forceFlag);
      result = result.abs();
    } catch (Exception e) {
      JshException.readFail(logger, e);
    ...
    if (null != rowArr && rowArr.size() > 0) {
      ...
      for (int i = 0; i < rowArr.size(); i++) {
        ...
        if (BusinessConstants.ENABLE_SERIAL_NUMBER_ENABLED.equals(material.getEnableSerialNumber())
            || BusinessConstants.ENABLE_BATCH_NUMBER_ENABLED.equals(
          ...
          if (BusinessConstants.SUB_TYPE_CHECK_ENTER.equals(depotHead.getSubType())
            ...
            throw new BusinessRunTimeException(
                ExceptionConstants.MATERIAL_STOCK_CHECK_ERROR_CODE,
        ...
        if (StringUtil.isExist(rowObj.get("snList"))) {
          ...
          if (StringUtil.isExist(rowObj.get("depotId"))) {
            ...
            if (snArray.length == operNum) {
              ...
              serialNumberService.addSerialNumberByBill(
                  depotHead.getType(),
                  depotHead.getSubType(),
                  depotHead.getNumber(),
                  materialExtend.getMaterialId(),
                  depotId,
                  depotItem.getSnList());
            } else {
              throw new BusinessRunTimeException(
                  ExceptionConstants.DEPOT_HEAD_SN_NUMBERE_FAILED_CODE,
          ...
          if (BusinessConstants.DEPOTHEAD_TYPE_IN.equals(depotHead.getType())
            ...
            // 序列号不能为空
            if (BusinessConstants.ENABLE_SERIAL_NUMBER_ENABLED.equals(
                material.getEnableSerialNumber())) {
              throw new BusinessRunTimeException(
        ...
        if (StringUtil.isExist(rowObj.get("batchNumber"))) {
          ...
          if (BusinessConstants.DEPOTHEAD_TYPE_IN.equals(depotHead.getType())
              || BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(depotHead.getType())) {
            // 批号不能为空
            if (BusinessConstants.ENABLE_BATCH_NUMBER_ENABLED.equals(
                material.getEnableBatchNumber())) {
              throw new BusinessRunTimeException(
          ...
          if ("add".equals(actionType)) {
            ...
            if (depotItem.getOperNumber().add(finishNumber).compareTo(preNumber) > 0) {
              throw new BusinessRunTimeException(
                  ...
                  String.format(
                      ExceptionConstants.DEPOT_HEAD_NUMBER_NEED_EDIT_FAILED_MSG, barCode));
            }
            ...
            BigDecimal preNumber =
                getPreItemByHeaderIdAndMaterial(
                        ...
                        depotItem.getMaterialExtendId(),
                        depotItem.getLinkId())
                    .getOperNumber();
            // 除去此单据之外的已入库|已出库
            BigDecimal realFinishNumber =
                getRealFinishNumber(
                    currentSubType,
                    depotItem.getMaterialExtendId(),
                    depotItem.getLinkId(),
          ...
          if (materialExtend.getLowDecimal() != null) {
            ...
            if ("零售".equals(depotHead.getSubType()) || "销售".equals(depotHead.getSubType())) {
              if (unitPrice.compareTo(materialExtend.getLowDecimal()) < 0) {
                throw new BusinessRunTimeException(
                    ExceptionConstants.DEPOT_HEAD_UNIT_PRICE_LOW_CODE,
        ...
        if (BusinessConstants.SUB_TYPE_SALES.equals(depotHead.getSubType())
          ...
          if (StringUtil.isNotEmpty(depotItem.getBatchNumber())) {
            depotItem.setPurchaseUnitPrice(
                getDepotItemByBatchNumber(depotItem.getBatchNumber()).getUnitPrice());
          }
        }
        ...
        if (StringUtil.isExist(rowObj.get("depotId"))) {
          ...
          if (!BusinessConstants.SUB_TYPE_PURCHASE_ORDER.equals(depotHead.getSubType())
            ...
            throw new BusinessRunTimeException(
                ExceptionConstants.DEPOT_HEAD_DEPOT_FAILED_CODE,
        ...
        if (BusinessConstants.SUB_TYPE_TRANSFER.equals(depotHead.getSubType())) {
          if (StringUtil.isExist(rowObj.get("anotherDepotId"))) {
            ...
            throw new BusinessRunTimeException(
                ExceptionConstants.DEPOT_HEAD_ANOTHER_DEPOT_FAILED_CODE,
        ...
        if (BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(depotHead.getType())) {
          BigDecimal stock =
              getStockByParam(depotItem.getDepotId(), depotItem.getMaterialId(), null, null);
          if (StringUtil.isNotEmpty(depotItem.getSku())) {
            // 对于sku商品要换个方式计算库存
            stock =
                getSkuStockByParam(
                    depotItem.getDepotId(), depotItem.getMaterialExtendId(), null, null);
          }
          BigDecimal thisBasicNumber =
              depotItem.getBasicNumber() == null ? BigDecimal.ZERO : depotItem.getBasicNumber();
          if (!systemConfigService.getMinusStockFlag() && stock.compareTo(thisBasicNumber) < 0) {
            throw new BusinessRunTimeException(
                ExceptionConstants.MATERIAL_STOCK_NOT_ENOUGH_CODE,
                String.format(
                    ExceptionConstants.MATERIAL_STOCK_NOT_ENOUGH_MSG, material.getName()));
          }
          // 出库时处理序列号
          if (!BusinessConstants.SUB_TYPE_TRANSFER.equals(depotHead.getSubType())) {
            ...
            if (BusinessConstants.ENABLE_SERIAL_NUMBER_ENABLED.equals(
              ...
              serialNumberService.checkAndUpdateSerialNumber(
                  depotItem,
                  depotHead.getNumber(),
                  userInfo,
                  StringUtil.toNull(depotItem.getSnList()));
            }
          }
        }
      ...
      throw new BusinessRunTimeException(
          ExceptionConstants.DEPOT_HEAD_ROW_FAILED_CODE,
          String.format(ExceptionConstants.DEPOT_HEAD_ROW_FAILED_MSG));
    }
  }
  /**
   * 判断单据的状态 通过数组对比：原单据的商品和商品数量（汇总） 与 分批操作后单据的商品和商品数量（汇总）
   *
   * @param depotHead
   * @return
   */
  ...
  public String getBillStatusByParam(DepotHead depotHead) {
    ...
    List<DepotItemVo4MaterialAndSum> linkList =
        depotItemMapperEx.getLinkBillDetailMaterialSum(depotHead.getLinkNumber());
  ...
  public void changeBillStatus(DepotHead depotHead, String billStatus) {
    ...
    try {
      ...
      logger.error(
          ...
          e);
      throw new BusinessRunTimeException(
          ExceptionConstants.DATA_WRITE_FAIL_CODE, ExceptionConstants.DATA_WRITE_FAIL_MSG);
    }
  }
    ...
    if (BusinessConstants.SUB_TYPE_ASSEMBLE.equals(subType)
      ...
      if (rowArr.size() > 1) {
        ...
        if (!"组合件".equals(firstMaterialType) || !"普通子件".equals(secondMaterialType)) {
          throw new BusinessRunTimeException(
              ExceptionConstants.DEPOT_HEAD_CHECK_ASSEMBLE_EMPTY_CODE,
  ...
  public List<DepotItemStockWarningCount> findStockWarningCount(
      Integer offset, Integer rows, String materialParam, List<Long> depotList) {
  ...
  public BigDecimal getSkuStockByParam(Long depotId, Long meId, String beginTime, String endTime)
    ...
    Boolean forceFlag = systemConfigService.getForceApprovalFlag();
    List<Long> depotList = depotService.parseDepotList(depotId);
    DepotItemVo4Stock stockObj =
        depotItemMapperEx.getSkuStockByParamWithDepotList(
            depotList, meId, forceFlag, beginTime, endTime);
    ...
    if (stockObj != null) {
      ...
      stockSum =
          inTotal
              ...
              .add(disAssemInTotal)
              .subtract(outTotal)
              .subtract(transfOutTotal)
              .subtract(assemOutTotal)
              .subtract(disAssemOutTotal);
    }
    return stockSum;
  }
   ...
   * @return
   */
  public BigDecimal getStockByParamWithDepotList(
      List<Long> depotList, Long mId, String beginTime, String endTime) throws Exception {
    Boolean forceFlag = systemConfigService.getForceApprovalFlag();
    // 初始库存
    BigDecimal initStock = materialService.getInitStockByMidAndDepotList(depotList, mId);
    // 盘点复盘后数量的变动
    BigDecimal stockCheckSum =
        depotItemMapperEx.getStockCheckSumByDepotList(
            depotList, mId, forceFlag, beginTime, endTime);
    DepotItemVo4Stock stockObj =
        depotItemMapperEx.getStockByParamWithDepotList(
            depotList, mId, forceFlag, beginTime, endTime);
    BigDecimal stockSum = BigDecimal.ZERO;
    if (stockObj != null) {
      BigDecimal inTotal = stockObj.getInTotal();
   ...
   * @return
   */
  public Map<String, BigDecimal> getIntervalMapByParamWithDepotList(
      List<Long> depotList, Long mId, String beginTime, String endTime) throws Exception {
    Boolean forceFlag = systemConfigService.getForceApprovalFlag();
  ...
  public void updateCurrentStockFun(Long mId, Long dId) throws Exception {
    if (mId != null && dId != null) {
      ...
      example
          .createCriteria()
          .andMaterialIdEqualTo(mId)
          .andDepotIdEqualTo(dId)
          .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
  ...
  public BigDecimal getFinishNumber(
      Long meId, Long id, Long headerId, Unit unitInfo, String materialUnit, String linkType) {
    Long linkId = id;
    String goToType = "";
    DepotHead depotHead = depotHeadMapper.selectByPrimaryKey(headerId);
   ...
   */
  @Transactional(value = "transactionManager", rollbackFor = Exception.class)
  public BigDecimal getRealFinishNumber(
    ...
    String linkNumber = depotHead.getNumber(); // 订单号
    BigDecimal count =
        depotItemMapperEx.getRealFinishNumber(meId, linkId, linkNumber, currentHeaderId, goToType);
    // 根据多单位情况进行数量的转换
    if (materialUnit.equals(unitInfo.getOtherUnit())
        && unitInfo.getRatio() != null
        && unitInfo.getRatio().compareTo(BigDecimal.ZERO) != 0) {
  ...
  public List<DepotItemVoBatchNumberList> getBatchNumberList(
      String number, String name, Long depotId, String barCode, String batchNumber) {
    return depotItemMapperEx.getBatchNumberList(
        StringUtil.toNull(number), name, depotId, barCode, batchNumber);
  }
