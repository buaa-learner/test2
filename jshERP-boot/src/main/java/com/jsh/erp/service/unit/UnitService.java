...
public class UnitService {
  ...
  @Resource private UnitMapper unitMapper;

  @Resource private UnitMapperEx unitMapperEx;
  ...
  private void parseNameByUnit(Unit unit) {
    ...
    if (StringUtil.isNotEmpty(unit.getOtherUnitTwo()) && unit.getRatioTwo() != null) {
      unitName +=
          "/"
              + "("
              + unit.getOtherUnitTwo()
              + "="
              + unit.getRatioTwo().toString()
              + unit.getBasicUnit()
              + ")";
      if (StringUtil.isNotEmpty(unit.getOtherUnitThree()) && unit.getRatioThree() != null) {
        unitName +=
            "/"
                + "("
                + unit.getOtherUnitThree()
                + "="
                + unit.getRatioThree().toString()
                + unit.getBasicUnit()
                + ")";
      }
    }
  ...
  public int batchDeleteUnitByIds(String ids) throws Exception {
    ...
    try {
      result =
          unitMapperEx.batchDeleteUnitByIds(
  ...
  public Long getUnitIdByParam(String basicUnit, String otherUnit, BigDecimal ratio) {
    ...
    example
        .createCriteria()
        .andBasicUnitEqualTo(basicUnit)
        .andOtherUnitEqualTo(otherUnit)
        .andRatioEqualTo(ratio)
        .andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
