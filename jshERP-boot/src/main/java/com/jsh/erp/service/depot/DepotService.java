...
public class DepotService {
  ...
  @Resource private DepotMapper depotMapper;

  @Resource private DepotMapperEx depotMapperEx;
  ...
  public int batchDeleteDepotByIds(String ids) throws Exception {
    ...
    try {
      result =
          depotMapperEx.batchDeleteDepotByIds(
  ...
  public int updateIsDefault(Long depotId) throws Exception {
    ...
    try {
      ...
      logService.insertLog(
          "仓库",
          BusinessConstants.LOG_OPERATION_TYPE_EDIT + depotId,
          ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest());
      result = 1;
    } catch (Exception e) {
      JshException.writeFail(logger, e);
